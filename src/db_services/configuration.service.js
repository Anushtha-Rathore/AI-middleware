    ...
    const pipeline = [
      ...
      },
      {
        $addFields: {
          ...
          connected_agents: { $ifNull: ["$connected_tools.connected_agents", {}] },
          variables_path: { $ifNull: ["$connected_tools.variables_path", {}] },
          variables_state: { $ifNull: ["$connected_tools.variables_state", {}] },
          built_in_tools: { $ifNull: ["$connected_tools.built_in_tools", []] },
          doc_ids: { $ifNull: ["$connected_tools.doc_ids", []] },
          web_search_filters: { $ifNull: ["$connected_tools.web_search_filters", []] },
          gtwy_web_search_filters: { $ifNull: ["$connected_tools.gtwy_web_search_filters", []] }
        }
      }
    ];
...
const getAllAgentsInOrg = async (org_id, folder_id, user_id, isEmbedUser) => {
  ...
  const processedAgents = agents.map((agent) => {
    ...
    agent.connected_agents = agent.connected_tools?.connected_agents || {};
    agent.variables_path = agent.connected_tools?.variables_path || {};
    agent.variables_state = agent.connected_tools?.variables_state || {};
    agent.doc_ids = agent.connected_tools?.doc_ids || [];
    agent.built_in_tools = agent.connected_tools?.built_in_tools || [];
    agent.web_search_filters = agent.connected_tools?.web_search_filters || [];
    agent.gtwy_web_search_filters = agent.connected_tools?.gtwy_web_search_filters || [];
